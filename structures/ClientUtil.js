...
class ClientUtil {
    ...
    subStrAfterChars(str, char, pos, end = '') {
        if (pos == 'b') { return str.substring(str.indexOf(char) + char.length + 1); }
        else if (pos == 'a') { return str.substring(0, str.indexOf(char)); }
        else if (pos == 'ab') {
            const subbed = str.substring(str.indexOf(char), str.indexOf(end));
            return subbed.substring(subbed.indexOf(char) + char.length + 1);
        }
        else { return str; }
    }
}
